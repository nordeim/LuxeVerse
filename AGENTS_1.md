...
Summary of Analysis
LuxeVerse is a Cinematic Luxury E-Commerce platform built with a cutting-edge stack:
 - Framework: Next.js 16 (App Router) & React 19.
 - Language: TypeScript 6 with strict erasableSyntaxOnly mode (no enums or namespaces).
 - Styling: Tailwind CSS v4 (CSS-first, zero config files, OKLCH palette).
 - Architecture: Pnpm monorepo with Turborepo, RSC-first rendering.
 - Backend: tRPC, Prisma 7, PostgreSQL, and NextAuth v5.

Key Instructional Context (GEMINI.md)
The generated GEMINI.md serves as the foundational mandate for all future interactions, enforcing:
 1. The Meticulous Approach: A strict 6-phase workflow (Analyze, Plan, Validate, Implement, Verify, Deliver).
 2. The Anti-Generic Mandate: Rejection of "AI slop," template grids, and safe defaults in favor of bespoke luxury design.
 3. Strict Technical Rules: Zero use of any or enum, mandatory selector discipline for Zustand, and pure CSS-first Tailwind v4 configuration.
 4. Monorepo Boundaries: Proper usage of @luxeverse/ui, @luxeverse/utils, and app-specific logic in apps/web.
